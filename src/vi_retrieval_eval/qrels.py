...
def build_gold_from_identity(num_q: int) -> List[List[int]]:
    """Build identity relevance labels.

    Args:
        num_q: Number of queries/documents.

    Returns:
        List[List[int]]: Gold labels where query i maps to document i.
    """
...
def _read_qrels_safely(
    qrels_path: str, qid_col: str, docid_col: str, rel_col: str
) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"Qrels must contain column(s) {missing}. Found: {list(qrels.columns)}"
        )

    # 3) coerce rel to int
    qrels[rel_col] = (
        pd.to_numeric(qrels[rel_col], errors="coerce").fillna(0).astype(int)
    )
    # strip id
...
def load_qrels(
    ...
    rel_col: str,
    df: pd.DataFrame,
    csv_qid_col: Optional[str],
    csv_docid_col: Optional[str],
) -> List[List[int]]:
    """Load qrels and align IDs with sampled dataframe indices.

    Args:
        qrels_path: Path to qrels file.
        qid_col: Query-id column name in qrels.
        docid_col: Document-id column name in qrels.
        rel_col: Relevance column name in qrels.
        df: Sampled evaluation dataframe.
        csv_qid_col: Optional query-id column in `df`.
        csv_docid_col: Optional document-id column in `df`.

    Returns:
        List[List[int]]: Gold document index lists per query index.
    """
    qrels = _read_qrels_safely(qrels_path, qid_col, docid_col, rel_col)

    # build ID mapping from the sampled df
    def _build_map(col_name: Optional[str], label: str) -> Tuple[dict, Optional[str]]:
        if col_name and col_name in df.columns:
            ...
            print(
                f"[WARN] csv_{label}_col='{col_name}' not found in CSV. Falling back to identity indices."
            )
    ...
    if missed_q or missed_d or mapped_pairs < total_pos:
        print(
            f"[WARN] Unmapped qrels: {missed_q} queries, {missed_d} docs. "
    ...
    for qi in range(len(df)):
        if qi in gold_map and gold_map[qi]:
            ...
            # fallback to identity mapping to avoid an empty gold list
            out.append([qi])
    return out


def rank_of_first_gold(
    scores_matrix: np.ndarray, gold_lists: List[List[int]]
) -> List[float]:
    """
    Return the 1-based rank of the best positive doc for each query; inf if none found.
    Uses an inverse permutation for O(1) lookup per gold index.
    """
    ...
    for i in range(Q):
        ...
        inv[order] = np.arange(1, N + 1)  # inv[j] = rank of doc j (1-based)
        best = float("inf")
